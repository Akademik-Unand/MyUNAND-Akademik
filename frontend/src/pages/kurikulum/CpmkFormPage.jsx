...
export const CpmkFormPage = () => {
  ...
  const mutations = useResourceMutations("cpmk-detail", {
    create: isSubCreate
      ? "Sub-CPMK berhasil ditambahkan."
      : "CPMK berhasil ditambahkan.",
    update: isChild
      ? "Sub-CPMK berhasil diperbarui."
      : "CPMK berhasil diperbarui.",
  });
  const saving =
    mutations.create.isPending || mutations.update.isPending || bulkSaving;
  const kurikulumId =
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      parent_cpmk_id: isSubCreate
        ? cpmkId
        : existing.data?.parent_cpmk_id || null,
  ...
  if (mk.isPending || (isEdit && existing.isPending))
    ...
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle={`${mk.data?.nama_resmi || ""} | ${mk.data?.kode_matakuliah || ""}${
          isSubCreate && parent.data?.nama_cpmk
            ? ` | Induk: ${parent.data.nama_cpmk}`
            : ""
        }`}
        breadcrumbs={[
          { label: "Kurikulum & MK" },
