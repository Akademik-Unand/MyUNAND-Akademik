...
export const CPMKKurikulumPage = () => {
  ...
  const matrix = useMemo(() => {
    const built = buildCpmkScpMatrix(
      cps.data || [],
      mkLinks.data || [],
      cpmkRows.data || [],
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row) =>
        row.matakuliah?.jumlah_sks_kurikulum ?? row.jumlah_sks_kurikulum,
    },
    {
      key: "nama_resmi",
  ...
  const mappingPending =
    tab === "mapping" &&
    kurikulumId &&
    (cps.isPending || mkLinks.isPending || cpmkRows.isPending);
    ...
    <div className="space-y-4">
      ...
      {tab === "mapping" && (
        <Card title="Mapping Sub-CPMK ke SCP">
          {!kurikulumId && (
            <p className="text-sm text-base-content/60">
              Pilih kurikulum lalu klik Terapkan untuk menampilkan matriks
              mapping.
            </p>
          )}
