...
export const LaporanCPViewPage = () => {
  ...
  const preview = useLaporanCpPreview(
    laporan?.kurikulum_id,
    laporan?.semester_id,
    {
  ...
  const rows = withGroupedCapaian(
    (preview.data || []).filter((row) => selected.has(row.id)),
  );
      ...
      <Card title="Informasi Laporan">
        <DetailList
          items={[
            ...
            {
              label: "Semester",
              value: laporan?.semester_id
                ? semesterDanSebelumnyaLabel(laporan?.semester)
                : "Semua semester",
            },
          ]}
        />
