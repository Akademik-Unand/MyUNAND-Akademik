...
import {
  evaluasiCapaianOptions,
  evaluasiCapaianSeries,
} from "../../helpers/evaluasiChart";
...
export const EvaluasiCPMKPage = () => {
  ...
  const evaluasiFilter = scoped
    ? { matakuliah_id: id, semester_id: semester }
    : undefined;
  const query = useResourceQuery("evaluasi-cpmk", {
    params: evaluasiFilter ? { filter: evaluasiFilter } : undefined,
    enabled: scoped,
  });

  if (scoped && query.isPending) {
    return <PageSkeleton showFilter={false} tableCols={8} />;
  }

  const rows = query.data ?? [];
  const chartOptions = evaluasiCapaianOptions(
    rows.map((row) => row.cpmk?.nama_cpmk || row.cpmk_id),
  );
    ...
    <MKSemesterLayout semester={semester} onSemesterChange={setSemester}>
      {!scoped ? (
        <DataGate
          ...
          message="Pilih semester di atas untuk melihat rangkuman evaluasi CPMK dan nilai peserta mata kuliah ini."
        />
      ) : (
        <Card title="Evaluasi CPMK Semester">
          <div className="tabs tabs-box mb-4 w-fit bg-base-200">
            <button
              type="button"
              className={`tab ${tab === "rangkuman" ? "tab-active" : ""}`}
              onClick={() => setTab("rangkuman")}
            >
              Rangkuman Evaluasi CPMK
            </button>
            <button
              type="button"
              className={`tab ${tab === "grafik" ? "tab-active" : ""}`}
            ...
            <button
              type="button"
              className={`tab ${tab === "nilai" ? "tab-active" : ""}`}
                  ...
                  {rows.map((row) => (
                    <tr key={row.id}>
                      <td className="font-medium">
                        {row.cpmk?.nama_cpmk || row.cpmk_id}
                      </td>
          ...
          {tab === "grafik" && (
            <div>
              <h3 className="mb-3 text-sm font-medium">
                Capaian Target CPMK dalam Grafik
              </h3>
              {rows.length ? (
                <Chart
