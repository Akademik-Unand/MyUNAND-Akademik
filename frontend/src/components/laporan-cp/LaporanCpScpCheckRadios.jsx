...
const matchesAction = (selected, groupRows, action) => {
  ...
  const selectedIds = groupRows
    .filter((row) => selected.has(row.id))
    .map((row) => row.id);
  return (
    matchIds.length > 0 &&
    matchIds.length === selectedIds.length &&
    matchIds.every((id) => selected.has(id))
  );
};
...
export const LaporanCpScpCheckRadios = ({ groupRows, selected, onChange }) => {
  ...
  const active = matchesAction(selected, groupRows, pickedAction)
    ? pickedKey
    : derived;
