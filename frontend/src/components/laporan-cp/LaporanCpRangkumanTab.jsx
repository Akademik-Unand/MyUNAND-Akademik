  ...
  if (!evaluasi.length) {
    return (
      <p className="text-sm text-base-content/60">
        Belum ada data nilai untuk dihitung.
      </p>
    );
  }
            ...
            {evaluasi.map((row) => (
              <tr key={row.cpmk_id}>
                ...
                <td className="whitespace-normal">
                  {row.cpl?.length ? row.cpl.join(", ") : "—"}
                </td>
                <td className="text-center">{pct(row.target_persen_lulus)}</td>
                <td className="text-center">
                  {row.target_nilai_min == null
                    ? "—"
                    : `${row.target_nilai_min}/100`}
                </td>
                <td className="text-center">
                ...
                <td className="align-top max-w-[10rem] whitespace-normal">
                  {row.evaluasi || ""}
                </td>
                {TINDAK_LANJUT_UNIT.map((unit) => (
                  <td
                    key={`tl-${row.cpmk_id}-${unit.key}`}
                    className="align-top max-w-[8rem] whitespace-normal"
                  >
