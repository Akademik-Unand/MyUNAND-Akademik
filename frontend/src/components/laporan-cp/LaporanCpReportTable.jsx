  ...
  if (!rows.length) {
    return (
      <p className="text-sm text-base-content/60">
        Belum ada mata kuliah yang dipilih pada laporan ini.
      </p>
    );
  }
      ...
      <table className="table table-sm w-full min-w-[96rem]">
        <thead ref={headRef} className="sticky top-0 z-30">
          <tr className="text-xs uppercase text-base-content/60">
            <th rowSpan={2} className="bg-base-100 align-bottom">
              CP
            </th>
            <th rowSpan={2} className="bg-base-100 align-bottom">
              SCP
            ...
            <th rowSpan={2} className="bg-base-100 align-bottom">
              Target Kelulusan CPMK
            </th>
            <th rowSpan={2} className="bg-base-100 align-bottom">
              Capaian Kelulusan CPMK
            </th>
            <th rowSpan={2} className="bg-base-100 align-bottom">
              Capaian Kelulusan SCP
            </th>
            <th rowSpan={2} className="bg-base-100 align-bottom">
              Capaian Kelulusan CP
            </th>
            <th rowSpan={2} className="bg-base-100 align-bottom">
              Evaluasi
            </th>
            <th colSpan={4} className="bg-base-100 text-center">
              Tindak Lanjut Sudah Dilakukan
            </th>
            <th colSpan={4} className="bg-base-100 text-center">
              Usulan Tindak Lanjut Sistemik
            </th>
          </tr>
          <tr className="text-xs uppercase text-base-content/60">
            {TINDAK_LANJUT_UNIT.map((unit) => (
              <th
                key={`tl-${unit.key}`}
                className="bg-base-100 font-normal normal-case"
              >
                {unit.label}
              </th>
            ))}
          ...
          {rows.map((row, idx) => {
            ...
            return (
              <tr key={row.id}>
                ...
                <td className="align-top max-w-xs whitespace-normal">
                  {row.dosen_label || "—"}
                </td>
                ...
                {scpSpan > 0 && (
                  <td
                    rowSpan={scpSpan > 1 ? scpSpan : undefined}
                    className="align-middle text-center bg-base-100"
                  >
                ...
                {cpSpan > 0 && (
                  <td
                    rowSpan={cpSpan > 1 ? cpSpan : undefined}
                    className="align-middle text-center bg-base-100"
                  >
                    {formatCapaian(row.capaian_cp)}
                  </td>
                )}
                <td className="align-top whitespace-normal max-w-xs">
                  {row.evaluasi || ""}
                </td>
                {TINDAK_LANJUT_UNIT.map((unit) => (
                  <td
                    key={`tl-${row.id}-${unit.key}`}
                    className="align-top whitespace-normal max-w-[8rem]"
                  >
