...
export const PenawaranForm = ({ values, onChange }) => {
  ...
  const addProgram = (event) => {
    ...
    if (
      !programId ||
      selectedPrograms.some((item) => item.program_studi_id === programId)
    )
      return;
    onChange({
      ...values,
      prodi_tujuan: [
        ...selectedPrograms,
        { program_studi_id: programId, kuota: null },
      ],
      prodi_akses_id: "",
    });
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      <ResourceSelect
        ...
        label="Semester Program Studi *"
        value={values.semester_prodi_id || ""}
        onChange={set("semester_prodi_id")}
        getLabel={semesterProdiLabel}
        required
      />
      <ResourceSelect
        resource="matakuliah"
        label="Mata Kuliah *"
        value={values.matakuliah_id || ""}
        onChange={set("matakuliah_id")}
        getLabel={(row) => `${row.kode_matakuliah} — ${row.nama_resmi}`}
        required
      />
      <Input
      ...
      {selectedOnly && (
        <div className="space-y-2 md:col-span-2">
          <ResourceSelect
            ...
            value={values.prodi_akses_id || ""}
            onChange={addProgram}
          />
          <div className="flex flex-wrap gap-2">
            {selectedPrograms.map((item) => (
              <button
                key={item.program_studi_id}
                type="button"
                className="badge badge-outline gap-1"
                onClick={() =>
                  onChange({
                    ...values,
                    prodi_tujuan: selectedPrograms.filter(
                      (row) => row.program_studi_id !== item.program_studi_id,
                    ),
                  })
                }
              >
