...
export const RekapCpExtraFilters = ({
  academicFilter = {},
  onApply,
  onReset,
}) => {
  ...
  const cpParams = useMemo(
    () =>
      academicFilter.kurikulum_id
        ? { filter: { kurikulum_id: academicFilter.kurikulum_id } }
        : {},
    [academicFilter.kurikulum_id],
  );
  ...
  const kelasParams = useMemo(() => {
    ...
    if (academicFilter.semester_id)
      filter.semester_id = academicFilter.semester_id;
    if (academicFilter.program_studi_id)
      filter.program_studi_id = academicFilter.program_studi_id;
    filter.has_peserta = "1";
    return { filter };
  }, [
    draft.matakuliah_id,
    academicFilter.semester_id,
    academicFilter.program_studi_id,
  ]);
