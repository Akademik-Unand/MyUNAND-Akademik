...
export const rekapCpColumns = [
  ...
  },
  {
    ...
    render: (row) =>
      row.capaian_target == null ? "%" : `${row.capaian_target}%`,
  },
  {
    key: "status_tercapai",
    header: "Status CP/SCP Tercapai",
    render: (row) => Number(row.status_tercapai) || 0,
  },
  {
    key: "sumber_nama",
    header: "Sumber Penilaian",
    render: (row) => row.sumber_nama || "—",
  },
    ...
    render: (row) =>
      Number(row.lulus) ? (
        <Badge
          ...
          size="sm"
          outline
          className="h-auto shrink-0 whitespace-nowrap px-2"
        >
          Lulus
        </Badge>
      ) : (
