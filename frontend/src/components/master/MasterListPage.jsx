...
export const MasterListPage = ({
  ...
  const closeModal = () => {
    if (busy || mutations.create.isPending || mutations.update.isPending)
      return;
  ...
  const openEdit = (row) =>
    setModal({ open: true, mode: "edit", values: { ...row } });
      ...
      <Modal
        ...
        footer={
          <FormActions
            ...
            isLoading={saving}
            onSubmitClick={() =>
              document.getElementById("master-form")?.requestSubmit()
            }
          />
        }
      >
