...
export const isLeafCpmk = (item, items = []) =>
  !parentIdsOf(items).has(item.id);
...
export const totalBobotMataKuliah = (items = []) =>
  items.reduce(
    (sum, item) =>
      isLeafCpmk(item, items) ? sum + sumberBobot(item.sumberPenilaian) : sum,
    0,
  );

export const bobotMelebihiMaks = (items = []) =>
  totalBobotMataKuliah(items) > MAX_MK_BOBOT + 0.01;
