...
export const toAcademicExtraFilter = (
  applied = EMPTY_ACADEMIC_FILTER,
  keys = [],
) => {
  ...
  if (allow("kurikulum") && applied.kurikulumId)
    filter.kurikulum_id = applied.kurikulumId;
  else if (allow("prodi") && applied.prodiId)
    filter.program_studi_id = applied.prodiId;
  else if (allow("departemen") && applied.departemenId)
    filter.departemen_id = applied.departemenId;
  else if (allow("fakultas") && applied.fakultasId)
    filter.fakultas_id = applied.fakultasId;

  if (allow("semester") && applied.semesterId)
    filter.semester_id = applied.semesterId;
...
export const academicScopeLock = (scope = {}) => {
  ...
  if (scope.level === "fakultas") {
    return {
      fakultasId: single(scope.fakultas_ids),
      departemenId: "",
      prodiId: "",
    };
  }
  if (scope.level === "departemen") {
    return {
      fakultasId: "",
      departemenId: single(scope.departemen_ids),
      prodiId: "",
    };
  }
...
export const cascadeAcademicOptions = (
  rows,
  ...
  const departemenRows = (rows.departemen || []).filter((row) => {
    ...
    if (scope.level === "departemen" && !departemenIds.has(row.id))
      return false;
    if (
      scope.level === "prodi" &&
      departemenIds.size &&
      !departemenIds.has(row.id)
    )
      return false;
    return true;
  });
  ...
  const prodiRows = (rows.prodi || []).filter((row) => {
    if (draft.departemenId && row.departemen_id !== draft.departemenId)
      return false;
    if (draft.fakultasId && !draft.departemenId) {
      if (
        row.fakultas_id !== draft.fakultasId &&
        row.departemen?.fakultas_id !== draft.fakultasId
      ) {
        return false;
      }
    }
    if (scope.level === "prodi" && !prodiIds.has(row.id)) return false;
    if (scope.level === "departemen" && !departemenIds.has(row.departemen_id))
      return false;
    return true;
  });
  ...
  const kurikulumRows = (rows.kurikulum || []).filter((row) => {
    ...
    if (
      !draft.prodiId &&
      scope.level === "prodi" &&
      !prodiIds.has(row.program_studi_id)
    )
      return false;
    return true;
  });
  ...
  return {
    ...
    departemen: toOptions(
      departemenRows,
      (row) => row.nama_resmi || row.nama_singkat || row.kode_departemen,
    ),
...
export const buildAcademicFilterFields = ({
  keys,
  draft,
  options,
  onChange,
