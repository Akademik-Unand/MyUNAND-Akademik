...
describe("consecutiveRowSpans", () => {
  it("merges consecutive student cells", () => {
    expect(consecutiveRowSpans(rows, (row) => row.student)).toEqual([
      4, 0, 0, 0, 1,
    ]);
  });
  ...
  it("merges consecutive MK cells under the same student", () => {
    expect(
      consecutiveRowSpans(rows, (row) => `${row.student}|${row.mk}`),
    ).toEqual([2, 0, 2, 0, 1]);
  });

  it("leaves unique sumber rows unmerged", () => {
    expect(consecutiveRowSpans(rows, (row) => row.sumber)).toEqual([
      1, 1, 1, 1, 1,
    ]);
  });
});
