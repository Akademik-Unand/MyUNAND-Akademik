...
describe("laporanCp helpers", () => {
  ...
  it("applies a radio choice only within the SCP group", () => {
    const next = applyMatchingInGroup(
      new Set(["c2:m2:s2"]),
      rows,
      (row) => row.is_transkrip,
    );
  ...
  it("detects the active radio from selected rows", () => {
    ...
    expect(activeCheckActionKey(new Set(["c1:m1:s1"]), rows, actions)).toBe(
      "transkrip:s1",
    );
    expect(
      activeCheckActionKey(new Set(["c1:m1:s1", "c2:m2:s2"]), rows, actions),
    ).toBe("all");
  });
