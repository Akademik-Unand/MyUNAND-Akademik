...
const assertCpmkBolehPunyaSumber = async (cpmkId, transaction) => {
  ...
  if (child) {
    throw new AppError(
      "Sumber penilaian hanya boleh pada CPMK atau Sub-CPMK yang tidak punya turunan",
      422,
    );
  }
};

const assertTotalBobotMk = async (
  cpmkId,
  { excludeSumberId, incomingBobot },
  transaction,
) => {
  ...
  const total =
    ...
    Number(incomingBobot || 0);
  if (total > MAX_MK_BOBOT + 0.01) {
    throw new AppError(
      "Total bobot sumber penilaian pada mata kuliah maksimal 100%",
      422,
    );
  }
};
