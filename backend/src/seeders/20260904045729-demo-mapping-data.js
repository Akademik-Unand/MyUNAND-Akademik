...
const requireId = (value, label) => {
  if (!value) {
    throw new Error(
      `Seeder demo mapping butuh ${label}. Jalankan seeder master akademik (002) dulu.`,
    );
  }
  return value;
};
...
const niuFor = (prodiKode, angkatan, index) => {
  ...
  return `${String(angkatan).slice(2)}${prodiKode.slice(0, 3)}${suffix}${prodiKode.slice(-2)}`.slice(
    0,
    20,
  );
};
...
module.exports = {
  async up(queryInterface) {
    ...
    const prodiMat = requireId(
      (
        await allRows(
          queryInterface,
          "SELECT id FROM program_studi WHERE kode_prodi = '44201' LIMIT 1",
        )
      )[0],
    ...
    const mkKurikulumPairs = new Set(
      (
        await allRows(
          queryInterface,
          "SELECT kurikulum_id, matakuliah_id FROM matakuliah_kurikulum",
        )
    ...
    linkMk(kurikulumSi, [
      { kode: "SI1101", status: "Wajib" },
      { kode: "SI2103", status: "Wajib" },
      ...catalog.SI_MATAKULIAH,
    ]);
    ...
    const cpByNama = mapBy(
      await allRows(queryInterface, "SELECT id, nama_cp FROM cp"),
      "nama_cp",
    );
    const scpByNama = mapBy(
      await allRows(queryInterface, "SELECT id, nama_scp FROM scp"),
      "nama_scp",
    );
    const scpByKey = {};
    ...
    const cpmkByKey = Object.fromEntries(
      cpmkExisting.map((item) => [
        cpmkKey(item.matakuliah_id, item.nama_cpmk),
        item.id,
      ]),
    );
    const cpmkRows = [];
    ...
    const sumberExisting = new Set(
      (
        await allRows(
          queryInterface,
          "SELECT cpmk_id, nama_sumber_penilaian FROM sumber_penilaian",
        )
    ...
    const addSumber = (cpmkId, nama, bobot) => {
      ...
      sumberRows.push(
        row(now, { cpmk_id: cpmkId, nama_sumber_penilaian: nama, bobot }),
      );
    };
    ...
    const applyCpmkSpecs = (specs) => {
      ...
      for (const spec of specs) {
        ...
        leafCountByMk[spec.mk] =
          (leafCountByMk[spec.mk] || 0) + (spec.hasSub ? spec.scp.length : 1);
      }
    ...
    for (const item of [...catalog.DOSEN_SI, ...catalog.DOSEN_MAT]) {
      ...
      dosenInserts.push(
        row(now, {
          ...
          nidn: item.nidn,
          nama: item.nama,
          program_studi_id: catalog.DOSEN_MAT.includes(item)
            ? prodiMat
            : prodiSi,
        }),
      );
    ...
    const dosenMatIds = catalog.DOSEN_MAT.map(
      (item) => dosenByNip[item.nip],
    ).filter(Boolean);

    const mhsByNiu = mapBy(
      await allRows(queryInterface, "SELECT id, niu FROM mahasiswa"),
      "niu",
    );
    const mhsInserts = [
    ...
    const bimbinganExisting = new Set(
      (
        await allRows(
          queryInterface,
          "SELECT dosen_id, mahasiswa_id, tahun_akademik FROM bimbingan_akademik",
        )
      ).map(
        (item) =>
          `${item.dosen_id}:${item.mahasiswa_id}:${item.tahun_akademik}`,
      ),
    );
    ...
    const ruangByKode = mapBy(
      await allRows(queryInterface, "SELECT id, kode FROM ruang"),
      "kode",
    );
    const ruangInserts = catalog.RUANG.filter(
      (item) => !ruangByKode[item.kode],
    ).map((item) => {
      const id = randomUUID();
    ...
    const planKelas = [
      ...[...catalog.SI_MATAKULIAH, { kode: "SI1101" }, { kode: "SI2103" }]
        .filter((item) => {
          const full = catalog.SI_MATAKULIAH.find(
            (rowItem) => rowItem.kode === item.kode,
          );
    ...
    for (const meta of kelasMeta) {
      ...
      if (dosenId && !dosenKelasExisting.has(`${dosenId}:${meta.id}`)) {
        ...
        const [jamMulai, jamSelesai] =
          catalog.JAM[meta.slot % catalog.JAM.length];
    ...
    const addDetil = (mahasiswaIds, sem) => {
      ...
      mahasiswaIds.forEach((mahasiswaId, index) => {
        ...
        for (const kelasId of picked) {
          ...
          krsDetilRows.push(
            row(now, { krs_id: krsId, kelas_id: kelasId, approved: "1" }),
          );
        }
      });
    };
    ...
    const nilaiExisting = new Set(
      (
        await allRows(
          queryInterface,
          "SELECT krs_detil_id, sumber_penilaian_id FROM nilai_mahasiswa",
        )
  ...
  async down(queryInterface) {
    ...
    const cpNames = [...catalog.SI_CP, ...catalog.MAT_CP].map(
      (item) => item.nama,
    );
    const scpNames = [...catalog.SI_CP, ...catalog.MAT_CP].flatMap((item) =>
      item.scp.map((scp) => scp.nama),
    );
    ...
    await queryInterface.bulkDelete("kurikulum", {
      nama: "Kurikulum OBE 2024 Matematika",
    });
  },
};
