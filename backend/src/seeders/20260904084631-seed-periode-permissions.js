...
const NEW_KEYS = ["periode"];

module.exports = {
  async up(queryInterface) {
    const now = new Date();
    const catalog = buildCatalog().filter((item) =>
      NEW_KEYS.includes(item.key),
    );
    ...
    const grantFor = (roleName, predicate) => {
      ...
      return catalog
        ...
        .filter(
          (row) =>
            row.permission_id &&
            !grantKeys.has(`${row.role_id}:${row.permission_id}`),
        );
    };

    const grants = [];
    for (const [roleName, predicate] of Object.entries(ROLE_GRANT_PREDICATES)) {
      grants.push(...grantFor(roleName, predicate));
    }
    if (grants.length) {
