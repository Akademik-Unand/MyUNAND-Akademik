...
const { getActivePa, assertActivePa } = require('../../helpers/activePa');
...
const INCLUDE = [
  {
    ...
    include: [
      ...
      { model: SemesterProdi, as: 'semesterProdi', include: [{ model: Semester, as: 'semester' }] },
    ],
  },
  {
    model: Kelas,
      ...
      { model: Matakuliah, as: 'matakuliah' },
      {
        ...
        include: [{
          ...
          include: [{
            model: SemesterProdi,
            as: 'semesterProdi',
            include: [{ model: ProgramStudi, as: 'programStudi' }, { model: Semester, as: 'semester' }],
          }],
        }],
      },
...
const getStudent = async (userId, transaction) => {
  const row = await Mahasiswa.findOne({
    include: [{ association: 'user', where: { id: userId }, attributes: [] }],
    transaction,
  });
...
const validateStudentLoad = async ({ krs, kelas, transaction }) => {
  ...
  if (usedSks + sks > krs.semesterProdi.sks_maksimal) {
...
const enroll = (userId, payload) =>
  sequelize.transaction(async (transaction) => {
    ...
    if (header.semesterProdi.program_studi_id === student.program_studi_id) {
    ...
    const semesterKe = Math.max(1, (header.semesterProdi.semester.tahun - student.angkatan) * 2 + 1);
    ...
    const krs = await Krs.findOne({
      ...
      include: [{ model: SemesterProdi, as: 'semesterProdi', where: { semester_id: header.semesterProdi.semester_id } }],
      transaction,
      lock: transaction.LOCK.UPDATE,
    });
    if (!krs) throw new AppError('KRS semester aktif belum tersedia', 409);
    await assertKrsPeriodForSemesterProdi(krs.semester_prodi_id);
    ...
    await validateStudentLoad({ krs, kelas, transaction });
...
const approvePa = (id, userId, payload) =>
  sequelize.transaction(async (transaction) => {
    const actor = await User.findByPk(userId, { attributes: ['id', 'dosen_id'], transaction });
    const row = await KrsDetil.findByPk(id, {
      include: [{ model: Krs, as: 'krs', include: [{ model: Mahasiswa, as: 'mahasiswa' }] }],
      transaction,
      lock: transaction.LOCK.UPDATE,
    });

    if (!row?.is_cross_enrollment) throw new AppError('Pengajuan lintas prodi tidak ditemukan', 404);
    if (row.cross_enrollment_status !== 'pending_pa') throw new AppError('Pengajuan sudah diproses', 409);

    const pa = await getActivePa(row.krs.mahasiswa_id, { transaction });
    if (!pa) throw new AppError('Mahasiswa belum memiliki dosen PA', 422);
    if (!actor?.dosen_id || actor.dosen_id !== pa.dosen_id) {
      throw new AppError('Hanya dosen PA mahasiswa tersebut yang dapat menyetujui', 403);
    }

    const now = new Date();
    if (payload.approved) {
      return row.update(
        { cross_enrollment_status: 'approved', pa_approved_by: userId, pa_approved_at: now, approved: '1' },
        { transaction }
      );
    }
    return row.update(
...
module.exports = { list, enroll, approvePa };
