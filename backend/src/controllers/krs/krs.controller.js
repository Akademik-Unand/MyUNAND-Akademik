...
const list = asyncHandler(async (req, res) => {
  const { rows, pagination } = await krsService.list(req.query, req.user);
  return success(res, {
...
const create = asyncHandler(async (req, res) => {
  const data = await krsService.create(req.body, req.user);
...
const updateDetilStatus = asyncHandler(async (req, res) => {
  const data = await krsService.updateDetilStatus(
    req.params.detilId,
    req.body.approved,
  );
  return success(res, {
...
const context = asyncHandler(async (req, res) => {
  const data = await krsService.getContext(req.user.id);
  return success(res, {
    message: "Konteks KRS mahasiswa berhasil diambil",
    data,
  });
});
